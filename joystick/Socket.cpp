...
Socket::Socket( uint16_t port, PortType type )
	: mSocketType( Server )
	, mPort( port )
	, mPortType( type )
	, mSocket( -1 )
	...
	if ( mSocket < 0 ) {
		...
		if ( mSocketType == Server ) {
			...
			if ( bind( mSocket, (SOCKADDR*)&mSin, sizeof(mSin) ) < 0 ) {
				std::cout << "Socket ( " << mPort << " ) error : " << strerror(errno) << "\n";
				close( mSocket );
				return -1;
			}
		}
	...
	if ( mPortType == TCP ) {
		if ( mSocketType == Server ) {
			...
			if ( connect( mSocket, (SOCKADDR*)&mSin, sizeof(mSin) ) < 0 ) {
				std::cout << "Socket connect to " << mHost << ":" << mPort << " error : " << strerror(errno) << "\n";
				close( mSocket );
				mSocket = -1;
				return -1;
			}
...
int Socket::Receive( void* buf, uint32_t len, int timeout )
{
	int ret = 0;
	memset( buf, 0, len );
	...
	if ( ret <= 0 ) {
		...
		std::cout << "Socket on port " << mPort << " disconnected ( " << ret << " : " << errno << ", " << strerror( errno ) << " )\n";
		close( mSocket );
		mSocket = -1;
		return -1;
	}
...
int Socket::Send( const void* buf, uint32_t len, int timeout )
{
	int ret = 0;
	...
	if ( ret < 0 and mPortType != UDP and mPortType != UDPLite ) {
		std::cout << "Socket on port " << mPort << " disconnected\n";
		close( mSocket );
		mSocket = -1;
		return -1;
	}
