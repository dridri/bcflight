...
//     #if defined (RF24_SPI_TRANSACTIONS)
    _SPI.beginTransaction(nRF24::SPISettings(RF24_SPI_SPEED, MSBFIRST, SPI_MODE0));
// 	#endif
    csn(LOW);
  }
...
RF24::RF24(uint16_t _cepin, uint16_t _cspin):
  ce_pin(_cepin),spidev(_cspin>=10?(_cspin/10):0), csn_pin(_cspin>=10?(_cspin-10):_cspin), p_variant(false),
...
RF24::RF24(uint16_t _cepin, uint16_t _cspin, uint32_t _spi_speed):
  ce_pin(_cepin),spidev(_cspin>=10?(_cspin/10):0),csn_pin(_cspin>=10?(_cspin-10):_cspin),spi_speed(_spi_speed),p_variant(false), payload_size(32), dynamic_payloads_enabled(false),addr_width(5)//,pipe0_reading_address(0) 
{
  pipe0_reading_address[0]=0;
}
#endif
...
void RF24::printDetails(void)
{
  ...
  if (csn_pin < BCM2835_SPI_CS_NONE ){
    ...
    csn_pin==7/*RPI_V2_GPIO_P1_26*/ ? " (CE1) Software Driven" : "" );
  }
