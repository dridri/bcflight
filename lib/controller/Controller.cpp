...
// 			mLink->Write( &uid, sizeof( uid ) );
	...
	if ( not mPauseControls ) {
		...
		// Always allow to disarm
		if ( not mSwitches[9] and mArmed ) {
			Disarm();
			mControls.arm = false;
			mTxFrame.WriteU8( CONTROLS );
			mTxFrame.Write( (uint8_t*)&mControls, sizeof(mControls) );
